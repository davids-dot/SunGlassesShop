...
a:hover {color:#FF6600; text-decoration: underline;}
  ...
  <body>
  <c:choose>
  	<c:when test="${requestScope.code eq 'success' }"><%--如果code是功能，它显示对号图片 --%>
  		<c:set var="img" value="/images/duihao.jpg"/>
  		<c:set var="title" value="温馨提示"/>
  	</c:when>
  	<c:when test="${requestScope.code eq 'error' }"><%--如果code是功能，它显示错号图片 --%>
  		<c:set var="img" value="/images/cuohao.png"/>
  		<c:set var="title" value="友情提醒"/>
  	</c:when>
...
<div class="divBody">
	...
	<div class="divContent">
	  <div style="margin: 20px;">
		...
		<br/>
		<br/>
		<span style="margin-left: 50px;"><a target="_top" href="${requestScope.login}">登录</a></span>
